import React from 'react';

const AmazonCTA = ({ text = "Buy on Amazon", className = "" }) => {
    return (
        <a href="https://a.co/d/blcy3Bv" target="_blank" rel="noopener noreferrer" className={`btn-primary ${className}`} style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '0.5rem',
            background: 'linear-gradient(135deg, #FF9900 0%, #FFB84D 100%)', // Amazon-ish orange but premium
            color: '#000',
            fontWeight: '700',
            textDecoration: 'none'
        }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path d="M7 18C5.9 18 5.01 18.9 5.01 20C5.01 21.1 5.9 22 7 22C8.1 22 9 21.1 9 20C9 18.9 8.1 18 7 18ZM17 18C15.9 18 15.01 18.9 15.01 20C15.01 21.1 15.9 22 17 22C18.1 22 19 21.1 19 20C19 18.9 18.1 18 17 18ZM7.17 14.75L7.2 14.63L8.1 13H15.55C16.3 13 16.96 12.59 17.3 11.97L21.16 4.96L19.42 4L15.55 11H8.53L8.4 10.73L6.16 6L5.21 4L4.27 2H1V4H3L6.6 11.59L5.25 14.03C5.09 14.32 5 14.65 5 15C5 16.1 5.9 17 7 17H19V15H7.42C7.29 15 7.17 14.89 7.17 14.75Z" />
            </svg>
            {text}
        </a>
    );
};

export default AmazonCTA;
